...
`deploy/k8s/production/postgres-resilience-policy.json` является canonical policy. `apps/coordinator/src/postgres-resilience.ts` валидирует provider snapshot и database observation, создаёт hash-only checkpoints, доказывает failover/restore и подписывает reports HMAC-SHA256. Kubernetes Job `agat-postgres-resilience-gate-v24` получает только runtime URL, CA, provider evidence и отдельный evidence key; admin/migration/tenant credentials ему не нужны.
...
| `agat_schema_migrations` | schema v24 | runtime manifest/admission contract |
| `agat_dr_canaries` | schema v24 | global, tenant-inaccessible restore/failover checkpoints |
...
2. Runtime schema v24 уже применена отдельной migration Job; coordinator replicas ещё не получают traffic.
...
kubectl wait --for=condition=Complete job/agat-postgres-resilience-gate-v24 \
  --namespace agat --timeout=300s
kubectl logs --namespace agat job/agat-postgres-resilience-gate-v24
```
...
Rollback application release выполняется только на schema-compatible v24 и не откатывает managed failover. После PITR clone production traffic остаётся на source; promote clone в authority относится к отдельному [region-loss/corruption incident runbook](./region-loss-dr.md). Нельзя dual-write source и clone.
