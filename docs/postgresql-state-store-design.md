...
PostgreSQL adapter активирован в релизе 1.7. Post-1.7 production-readiness этапы добавили canonical offline migration, DDL-free Job boundary, managed-resilience contract и текущую schema v24 с S3 artifact outbox и region-loss marker: coordinator создаёт bounded `pg` pools, но только валидирует schema/admission/activation contract; DDL выполняет отдельная Job под owner role.
...
- SQLite placeholders/небольшой dialect subset нормализуются в bridge; PostgreSQL schema v24 устанавливает fleet objects, RLS, audit/storage triggers, catalog manifest, admission/cell markers и tenant-inaccessible DR/activation ledger.
...
Provider-neutral gate проверяет multi-AZ, synchronous standby, automatic failover, private TLS endpoint, PITR retention/freshness и два distinct SLO approver. Текущая schema v24 сохраняет checkpoints и HMAC reports, доказывающие actual failover и exact PITR boundary; полный contract и per-cluster qualification описаны в [Managed PostgreSQL](./managed-postgresql-resilience.md).
...
Schema v23 ввела S3-compatible bytes authority с transactional PostgreSQL metadata/outbox, exact-version delete, retention/legal hold и bounded reconciliation; contract сохраняется в текущей v24. Полный contract: [S3 Artifact Store](./s3-artifact-store-lifecycle.md).

### Residency-aware region-loss — готово

Schema v24 связывает isolated PostgreSQL restore, version-preserving S3 replica и Temporal recovery через sealed evidence, whole-cell policy и exact database snapshots. Monotonic `write_epoch`/activation marker блокирует stale source runtime; failback является новой обратной активацией. Полный contract: [Region-loss DR](./region-loss-dr.md).
...
| DDL-free runtime role | schema v24 Job, manifest/admission и negative E2E готовы | managed-IaC role provisioning |
