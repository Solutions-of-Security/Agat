...
Используйте [приватную форму GitHub](https://github.com/Solutions-of-Security/Agat/security/advisories/new)
(`Security → Report a vulnerability`). Приватный приём сообщений включён.
Не раскрывайте эксплуатационные детали, секреты или персональные данные в публичном issue.

Укажите tag/commit, компонент и конфигурацию, минимальные шаги воспроизведения,
предполагаемое влияние и обезличенные доказательства. Не передавайте рабочие credentials
или пользовательские документы. Детали уточняются в приватном обсуждении;
согласованное раскрытие и исправление оформляются через GitHub Security Advisory.
Срок реакции зависит от серьёзности и доступности команды; гарантированный SLA не заявлен.

| Версия | Поддержка исправлений безопасности |
| --- | --- |
| Последний опубликованный preview серии 1.7 | Текущая поддерживаемая поставка; обновляйтесь до последнего исправления |
| Старые snapshots и прошлые preview | Исправления в текущей поставке; backport не гарантирован |
| Невыпущенные 1.8/1.9 | План разработки, не поддерживаемые релизы |

Обычные ошибки направляйте по [руководству поддержки](./SUPPORT.md).
Далее описаны реализованные механизмы и границы безопасности платформы.
