...
1. Откройте `Создание → Агенты → Маркетплейс`.
...
7. Созданная копия появится на вкладке `Настроенные`, в Prompt Registry и в конструкторах запуска/процесса.
...
- вкладки настроенных агентов/маркетплейса: `apps/web/src/components/AgentsPage.tsx`;
