...
DDL-free boundary введена schema v21 и сохраняется в текущей schema v23. PostgreSQL schema применяется только отдельной migration Job. Coordinator runtime больше не выполняет `CREATE`, `ALTER`, policy/trigger installation или grants при startup: он подключается DDL-free system role, проверяет immutable catalog manifest, privilege boundary и успешный connection admission marker, затем выполняет только application DML.
...
|---|---|---|
| `apps/coordinator/src/postgres-schema-migrator.ts` | текущая schema v23 | exclusive migration, runtime validation, admission finalize |
...
| `deploy/k8s/docker-desktop/coordinator-postgres-migration.yaml` | Job v23 | admin bootstrap и schema/admission Jobs |
...
4. `agat_schema_migrations` получает текущую version `23`, contract ID и SHA-256 catalog manifest; admission становится `pending`.
...
`npm run k8s:up` для существующего контура сначала масштабирует coordinator до нуля, удаляет текущие v23 и предыдущие v22 versioned Jobs, применяет manifests и ждёт:

1. `agat-artifact-store-bootstrap-v23` создаёт local bucket и включает versioning;
2. `agat-postgres-role-bootstrap-v23` с admin Secret;
3. `agat-postgres-schema-v23` только с migration/runtime/tenant URLs;
4. schema v23 + `admission_status=passed` в coordinator init container;
...
| process lost после DDL commit до marker pass | текущая schema v23, admission `pending` | безопасно rerun всей Job; coordinator остаётся заблокирован |
