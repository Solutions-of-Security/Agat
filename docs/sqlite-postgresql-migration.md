...
`fleet:migrate-state` переносит одну остановленную Agat HA-cell с текущей schema v25 из SQLite в новую PostgreSQL database. Это canonical offline cutover: dual-write, online catch-up и автоматическое переключение трафика не входят в контракт.
...
| MIG-05 | SQLite schema кроме v25 | exact-version fail-closed | отдельный source upgrade/rehearsal / Application owner |
...
Schema migration Job/role, DDL-free runtime, managed PostgreSQL resilience, S3 artifact authority, residency-aware region-loss DR, OCI/runtime attestation и SIEM retention/DLQ реализованы последующими этапами. Для production остаётся qualification конкретного deployment по [roadmap](./roadmap.md), а не ещё одна repository migration capability.
